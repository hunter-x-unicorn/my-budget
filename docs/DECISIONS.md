...
`seedDefaultsIfEmpty` — **internal** mutation, планируется из `categories.list` при пустом списке (`scheduler.runAfter(0, …)`). Не вызывается из `AddTab` / `AccountTab`.

## ensureDefaults удалён из public API

Публичная mutation `ensureDefaults` заменена internal seed; UI не инициирует сид.
